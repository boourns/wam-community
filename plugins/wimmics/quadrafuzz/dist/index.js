...
class QuadrafuzzNode extends CompositeAudioNode {
  /**
   * @type {ParamMgrNode}
   */
  ...
  connectNodes() {
    ...
    for (var i = 0; i < filters.length; i++) {
      ...
      this.wetGainNode.connect(filters[i]); //this.wetGainNode.connect(this.overdrives[i]);
      //filters[i].connect(this._output);

      filters[i].connect(this.overdrives[i]);
      this.overdrives[i].connect(this._output); //filters[i].connect(this._output);
    }
  }
  /**
  ...
  getDistortionCurve(k) {
    console.log("GET DISTORSION CURVE k = ".concat(k));
    var {
      sampleRate
    } = this.context;
  ...
  set lowGain(_lowGain) {
    console.log("set lowGain : ".concat(_lowGain));
    if (!this.isInRange(_lowGain, 0, 1)) return; //this.params.lowgain = _lowGain;
  ...
  set midLowGain(_midLowGain) {
    ...
    if (!this.isInRange(_midLowGain, 0, 1)) return; // this.params.midlowgain = _midLowGain;
  ...
  set midHighGain(_midHighGain) {
    ...
    if (!this.isInRange(_midHighGain, 0, 1)) return; // this.params.midhighgain = _midHighGain;
  ...
  set highGain(_highGain) {
    ...
    if (!this.isInRange(_highGain, 0, 1)) return; //this.params.highgain = _highGain;
...
var knobImg = "assets/MiniMoog_Main.png";
...
class QuadrafuzzHTMLElement extends HTMLElement {
  ...
  constructor(plugin) {
    ...
    _defineProperty(this, "updateStatus", status => {
  ...
  setResources() {
    ...
    background.src = getAssetUrl(backgroundImg); //background.src = bgImage;

    background.style = 'border-radius : 5px;'; // Setting up the knobs imgs, those are loaded from the assets
  ...
  setKnobs() {
    this.shadowRoot.querySelector('#knob1').addEventListener('input', e => {
      this.plugin.audioNode.setParamsValues({
        lowGain: e.target.value
      });
    });
    this.shadowRoot.querySelector('#knob2').addEventListener('input', e => {
      this.plugin.audioNode.setParamsValues({
        midLowGain: e.target.value
      });
    });
    this.shadowRoot.querySelector('#knob3').addEventListener('input', e => {
      this.plugin.audioNode.setParamsValues({
        midHighGain: e.target.value
      });
    });
    this.shadowRoot.querySelector('#knob4').addEventListener('input', e => {
      this.plugin.audioNode.setParamsValues({
        highGain: e.target.value
      });
    });
  }
  ...
  setSwitchListener() {
    ...
    var {
      plugin
    } = this; // by default, plugin is disabled

    plugin.audioNode.setParamsValues({
      enabled: 1
    });
...
class QuadrafuzzPlugin extends WebAudioModule_default {
  constructor() {
    super(...arguments);
  ...
  createAudioNode(initialState) {
    ...
    return _asyncToGenerator(function* () {
      var quadrafuzzNode = new QuadrafuzzNode(_this3.audioContext);
      var internalParamsConfig = {
        ...
        midLowGain: {
          ...
          maxValue: 1,
          onChange: value => {
            quadrafuzzNode.midLowGain = value;
          }
        },
        midHighGain: {
          defaultValue: 0.5,
          ...
          maxValue: 1,
          onChange: value => {
            quadrafuzzNode.midHighGain = value;
          }
        },
        highGain: {
          defaultValue: 0.5,
...
export { QuadrafuzzPlugin as default };
